...
class ConversationService:
    ...
    def process_message(self, channel_id: str, message: str, user_id: str) -> str:
        logger.info(
            "Processing message",
            extra={"channel_id": channel_id, "text": message, "user_id": user_id},
        )
    ...
    def process_message_background_check(self, channel_id: str, message: str, user_id: str) -> str:
        logger.info(
            "Processing message",
            extra={"channel_id": channel_id, "text": message, "user_id": user_id},
        )
        response = self.api_client.send_message_background_check(channel_id, message, user_id)
        logger.info("Received response from API client", extra={"response": response})
        return response["message"]